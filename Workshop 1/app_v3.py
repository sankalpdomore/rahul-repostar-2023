...
def accounts_data(positive_list=None):
    ...
    for i in positive_list:
        ...
        #     acc_ids.extend(list(ac_brands[ac_brands["Brand"]==value]["Account Id"].values))
            ...
            continue
    return acc[acc["Id"].isin(acc_ids)].drop(["label","Birthdate","Audience credibility","Relevant tags"],axis=1)
...
def get_recommendations(selections_dict, model, user_lookup, display_n=25):
    ...
    for node_id, value in model.most_similar_cosmul(**selections_dict, topn=100000):
        ...
        if node_type == 'Account':
            ...
            pass
        elif(node_type in filter_nodes):
    ...
    try:
        ...
        if len(users_df)!=0:
            results_tables.extend([
                html.H4("Accounts.csv"),
                dbc.Table.from_dataframe(users_df, striped=True, bordered=True, hover=False)
            ])
...
server = app.server
app.title = 'SocialTalk-Semantic Search'
...
print("User_lookup")
final = pd.read_csv('final_app_input_df_1.csv')
print("Final_df")
